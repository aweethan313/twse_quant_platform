...
def api_list_strategies(db: Session = Depends(get_db)):
    # FORWARD_ACCOUNTS_ONLY:排除回測帳戶(id >= 100)
    accs = db.query(StrategyAccount).filter(
        StrategyAccount.id >= 11, StrategyAccount.id < 100).all()
...
def api_strategy_registry():
    ...
    try:
        rows = db.execute(_t(
            ...
            " WHERE a.id >= 11 AND a.id < 100 ORDER BY a.id"
        )).fetchall()
...
def api_monthly_race(start_date: str = None):
    ...
    db = SessionLocal()
    try:
        acct_ids = db.execute(_t("SELECT id, name, initial_cash FROM strategy_accounts WHERE id >= 11 AND id < 100")).fetchall()
        results = []
...
def api_monthly_drawdown(start_date: str = None):
    ...
    db = SessionLocal()
    try:
        accounts = db.execute(_t("SELECT id, name FROM strategy_accounts WHERE id >= 11 AND id < 100")).fetchall()
        result = []
        for aid, aname in accounts:
            rows = db.execute(_t("""
                SELECT snap_date, total_equity FROM equity_curve
...
def api_risk_adjusted_ranking(start_date: str = None):
    ...
    try:
        ...
        accounts = db.execute(_t(
            "SELECT id, name FROM strategy_accounts WHERE id >= 11 AND id < 100"
        )).fetchall()
...
def api_strategy_account_metrics(account_id: int, start_date: str = None):
    ...
    try:
        ...
        # 成交統計：重播 fills 歷史算已實現損益（不可查 positions——賣出後已刪）
        fills = db.execute(_t("""

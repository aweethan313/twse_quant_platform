...
ACTION_COLS = ["signal", "action", "decision", "side", "recommendation"]
...
def looks_like_signal_table(t: TableInfo) -> bool:
    name = t.name.lower()
    has_signal_name = any(k in name for k in ["signal", "decision", "alert", "order"])
    ...
    for r in rows:
        ...
        signals.append(
            SignalRow(
                ...
                extra={k: r[k] for k in r.keys() if k not in {"rowid"}},
            )
        )
    return signals
...
def find_previous_completed_bar(
    ...
    # 保守假設：signal 當下只能使用上一根「已完成」的一分鐘K。
    ...
    sql = f"""
        ...
        LIMIT 1
    """
    return conn.execute(sql, (code, latest_allowed_ts.isoformat(sep=" "))).fetchone()
...
def find_same_minute_bar(
    ...
    sql = f"""
        ...
        LIMIT 1
    """
    return conn.execute(sql, (code, minute_start.isoformat(sep=" "))).fetchone()
...
def audit_signals(
    ...
    for s in signals:
        ...
        if not s.code:
            issues.append(make_issue("HIGH", "missing_code", s, "signal 缺少股票代號"))

        if s.ts is None:
            issues.append(make_issue("HIGH", "invalid_timestamp", s, f"無法解析時間戳：{s.ts_raw}"))
            continue
        ...
        if minute_table is None:
            issues.append(
                make_issue(
                    ...
                    s,
                    "找不到可用的一分鐘K資料表，無法驗證 signal 是否偷看未來",
                )
            )
            continue
        ...
        if prev_bar is None:
            issues.append(
                make_issue(
                    ...
                    s,
                    "signal 發生前找不到上一根已完成分K；策略可能沒有可用當下資料",
                )
            )
        ...
        if same_minute_bar is not None and s.ts.second == 0:
            issues.append(
                make_issue(
                    ...
                    s,
                    "signal 時間剛好等於該分鐘K時間；若使用該分鐘 close，可能偷看未來",
                )
            )
    ...
    for (code, day), rows in daily_actions.items():
        ...
        if "BUY" in actions and "SELL" in actions:
            first_buy = min((r for r in rows if r.action == "BUY"), key=lambda x: x.ts or datetime.max)
            first_sell = min((r for r in rows if r.action == "SELL"), key=lambda x: x.ts or datetime.max)
            issues.append(
                AuditIssue(
                    severity="HIGH",
...
def make_issue(severity: str, issue_type: str, s: SignalRow, message: str) -> AuditIssue:
    return AuditIssue(
        severity=severity,
...
def write_md(
    ...
    if issue_type_counts:
        ...
        lines.append("|---|---:|")
        for k, v in sorted(issue_type_counts.items(), key=lambda item: (-item[1], item[0])):
            lines.append(f"| {k} | {v} |")
    else:
...
def main() -> None:
    ...
    if not signal_tables:
        print("[WARN] 找不到 signal / decision / alert / order 類型資料表。")
